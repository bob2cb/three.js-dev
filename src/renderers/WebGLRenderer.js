...
function WebGLRenderer( parameters ) {
	...
	function initGLContext() {
		...
		//存储纹理的最大尺寸、着色器的精度值等信息
		capabilities = new WebGLCapabilities( _gl, extensions, parameters );
		//WebGL可以说是一个状态机，程序中多个API控制着多种功能的开启和显示，而WebGLState.js算是render中最最重要的脚本了，总体控制着程序的渲染工作
		state = new WebGLState( _gl, extensions, paramThreeToGL );
		//剪裁测试用于限制绘制区域。我们可以指定一个矩形的剪裁窗口，当启用剪裁测试后，只有在这个窗口之内的像素才能被绘制，其它像素则会被丢弃。
		//可以通过glScissor(x, y, width, height)来指定一个位置在(x, y)，宽度为width，高度为height的剪裁窗口。;
		//OpenGL窗口坐标是以左下角为(0, 0)，右上角为(width, height)的
